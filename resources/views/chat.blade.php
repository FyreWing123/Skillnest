<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pesan - SkillNest</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <style>
        html, body { height: 100%; overflow: hidden; }

        #chat-messages::-webkit-scrollbar,
        #contact-list::-webkit-scrollbar { width: 4px; }
        #chat-messages::-webkit-scrollbar-track,
        #contact-list::-webkit-scrollbar-track { background: transparent; }
        #chat-messages::-webkit-scrollbar-thumb,
        #contact-list::-webkit-scrollbar-thumb { background: #DCE7FB; border-radius: 99px; }

        @keyframes msgIn {
            from { opacity: 0; transform: translateY(6px); }
            to   { opacity: 1; transform: translateY(0); }
        }
        .msg-bubble { animation: msgIn 0.18s ease-out; }

        @keyframes bounce {
            0%, 80%, 100% { transform: translateY(0); }
            40%            { transform: translateY(-4px); }
        }
        .dot { animation: bounce 1.2s infinite; }
        .dot:nth-child(2) { animation-delay: 0.2s; }
        .dot:nth-child(3) { animation-delay: 0.4s; }
    </style>
</head>

<body class="bg-[#F6FAFF]">

<div class="flex h-screen overflow-hidden">

    {{-- ══════════════════════════════════════════
         PANEL 1 — NAVIGASI SIDEBAR
    ══════════════════════════════════════════ --}}
    <aside class="w-64 flex-shrink-0 bg-white border-r border-[#E2E8F0] flex flex-col p-6">

        <a href="{{ route('home') }}" class="flex items-center gap-3 mb-10">
            <img src="{{ asset('images/skillnestlogo.png') }}" class="h-10" alt="SkillNest">
            <span class="text-xl font-bold text-[#0F172A]">SkillNest</span>
        </a>

        <nav class="space-y-2 flex-1">
            @if(auth()->user()->role === 'mahasiswa')
                <a href="{{ route('dashboard.mahasiswa') }}" class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Dashboard</a>
                <a href="{{ route('profile.mahasiswa') }}"  class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Profil Saya</a>
                <a href="{{ route('portfolio.mahasiswa') }}" class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Portfolio</a>
                <a href="{{ route('layanan.saya') }}"        class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Layanan Saya</a>
            @else
                <a href="{{ route('dashboard.umkm') }}"     class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Dashboard</a>
                <a href="{{ route('profile.umkm') }}"       class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Profil UMKM</a>
                <a href="{{ route('cari.mahasiswa') }}"     class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Cari Mahasiswa</a>
                <a href="{{ route('pesanan.umkm') }}"       class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Pesanan Saya</a>
                <a href="{{ route('favorit.umkm') }}"       class="block rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-100">Favorit</a>
            @endif
            <a href="{{ route('chat') }}" class="block rounded-xl bg-[#EAF2FF] px-4 py-3 font-semibold text-[#1846A3]">Pesan</a>
        </nav>

    </aside>

    {{-- ══════════════════════════════════════════
         PANEL 2 — DAFTAR PERCAKAPAN
    ══════════════════════════════════════════ --}}
    <div class="w-80 flex-shrink-0 bg-white border-r border-[#E2E8F0] flex flex-col">

        <div class="p-5 border-b border-[#E2E8F0]">
            <h2 class="text-lg font-bold text-[#0F172A]">Pesan</h2>
            <p class="text-xs text-slate-400 mt-0.5">{{ auth()->user()->nickname ?? auth()->user()->name }}</p>
        </div>

        <div class="px-4 py-3 border-b border-[#E2E8F0]">
            <div class="relative">
                <svg class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
                <input type="text" placeholder="Cari percakapan..."
                    class="w-full rounded-xl bg-[#F6FAFF] border border-[#E2E8F0] pl-9 pr-4 py-2.5 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:border-[#2563EB]"
                    oninput="filterContacts(this.value)">
            </div>
        </div>

        <div id="contact-list" class="flex-1 overflow-y-auto">
            @forelse($conversations as $conv)
                @php
                    $myId   = auth()->id();
                    $other  = $conv->otherUser($myId);
                    $last   = $conv->lastMessage;
                    $unread = $conv->unreadCount($myId);
                    $initial= strtoupper(substr($other->nickname ?? $other->name, 0, 2));
                    $isActive = $active && $active->id === $conv->id;
                @endphp
                <button
                    id="contact-{{ $conv->id }}"
                    onclick="openConversation({{ $conv->id }})"
                    data-name="{{ strtolower($other->name) }} {{ strtolower($other->nickname ?? '') }}"
                    class="contact-item w-full flex items-center gap-3 px-4 py-3.5 hover:bg-[#F6FAFF] transition text-left border-b border-[#F1F5F9]
                           {{ $isActive ? 'bg-[#EAF2FF] border-l-2 border-l-[#2563EB]' : '' }}"
                >
                    <div class="h-12 w-12 rounded-full bg-[#EAF2FF] flex items-center justify-center text-[#1846A3] font-bold text-base flex-shrink-0">
                        {{ $initial }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between">
                            <p class="text-sm font-semibold text-[#0F172A] truncate">
                                {{ $other->nickname ?? explode(' ', $other->name)[0] }}
                            </p>
                            <span class="text-[10px] text-slate-400 flex-shrink-0 ml-2">
                                {{ $last ? $last->created_at->diffForHumans(null, true) : '' }}
                            </span>
                        </div>
                        <div class="flex items-center justify-between mt-0.5">
                            <p class="text-xs text-slate-500 truncate pr-2">
                                {{ $last ? Str::limit($last->body, 40) : 'Mulai percakapan...' }}
                            </p>
                            @if($unread > 0)
                                <span id="badge-{{ $conv->id }}" class="flex-shrink-0 h-5 w-5 rounded-full bg-[#2563EB] text-white text-[10px] font-bold flex items-center justify-center">
                                    {{ $unread }}
                                </span>
                            @endif
                        </div>
                    </div>
                </button>
            @empty
                <div class="flex flex-col items-center justify-center h-full text-center px-6 py-10">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-slate-200 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                    </svg>
                    <p class="text-sm text-slate-400 font-medium">Belum ada percakapan.</p>
                    @if(auth()->user()->role === 'umkm')
                        <a href="{{ route('cari.mahasiswa') }}" class="mt-3 text-xs font-semibold text-[#2563EB] hover:underline">
                            Cari mahasiswa →
                        </a>
                    @endif
                </div>
            @endforelse
        </div>

    </div>

    {{-- ══════════════════════════════════════════
         PANEL 3 — RUANG CHAT
    ══════════════════════════════════════════ --}}
    <div class="flex-1 flex flex-col overflow-hidden">

        {{-- TOP BAR --}}
        <header id="chat-header" class="flex-shrink-0 flex items-center gap-4 border-b border-[#E2E8F0] bg-white px-6 py-4">
            <div class="h-11 w-11 rounded-full bg-[#EAF2FF] flex items-center justify-center text-[#1846A3] font-bold text-base flex-shrink-0">
                <span id="chat-initial">{{ $active ? strtoupper(substr($active->otherUser(auth()->id())->nickname ?? $active->otherUser(auth()->id())->name, 0, 2)) : '--' }}</span>
            </div>
            <div class="flex-1 min-w-0">
                <p id="chat-name" class="text-sm font-bold text-[#0F172A]">
                    {{ $active ? ($active->otherUser(auth()->id())->nickname ?? explode(' ', $active->otherUser(auth()->id())->name)[0]) : 'Pilih percakapan' }}
                </p>
                <p id="chat-status" class="text-xs text-slate-400">
                    @if($active)
                        @php $otherUser = $active->otherUser(auth()->id()); @endphp
                        {{ $otherUser->role === 'mahasiswa' ? 'Mahasiswa' : ($otherUser->nama_usaha ?? 'UMKM') }}
                    @endif
                </p>
            </div>
        </header>

        {{-- NOTICE BANNER --}}
        <div class="flex-shrink-0 bg-[#FFF8E7] border-b border-[#FDE68A] px-6 py-2.5">
            <p class="text-xs text-[#92400E] text-center font-medium">
                💡 Gunakan chat ini untuk negosiasi & diskusi. Pembayaran di luar platform? Bisa juga via WhatsApp setelah deal.
            </p>
        </div>

        {{-- MESSAGES --}}
        <div id="chat-messages" class="flex-1 overflow-y-auto px-6 py-5 bg-[#F6FAFF]">

            @if(!$active)
                {{-- Empty state --}}
                <div class="flex flex-col items-center justify-center h-full text-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-slate-200 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                    </svg>
                    <p class="text-slate-400 font-medium">Pilih percakapan untuk mulai chat</p>
                </div>
            @else
                <div id="messages-inner" class="space-y-4">
                    <div class="flex items-center gap-3">
                        <div class="flex-1 h-px bg-[#E2E8F0]"></div>
                        <span class="text-xs font-semibold text-[#94A3B8]">Memuat pesan...</span>
                        <div class="flex-1 h-px bg-[#E2E8F0]"></div>
                    </div>
                </div>

                {{-- Typing indicator --}}
                <div id="typing-indicator" class="items-end gap-2.5 mt-4" style="display:none">
                    <div class="rounded-3xl rounded-bl-lg bg-white border border-[#DCE7FB] px-4 py-3 shadow-sm inline-block">
                        <div class="flex items-center gap-1.5">
                            <span class="dot h-2 w-2 rounded-full bg-[#94A3B8]"></span>
                            <span class="dot h-2 w-2 rounded-full bg-[#94A3B8]"></span>
                            <span class="dot h-2 w-2 rounded-full bg-[#94A3B8]"></span>
                        </div>
                    </div>
                </div>
            @endif

        </div>

        {{-- INPUT AREA --}}
        <div class="flex-shrink-0 border-t border-[#E2E8F0] bg-white px-6 py-4">
            <div class="flex items-end gap-3">

                <div class="flex-1">
                    <textarea
                        id="message-input"
                        rows="1"
                        placeholder="{{ $active ? 'Tulis pesan...' : 'Pilih percakapan dahulu...' }}"
                        {{ !$active ? 'disabled' : '' }}
                        class="w-full resize-none rounded-2xl border border-[#DCE7FB] bg-[#F6FAFF] px-5 py-3 text-sm text-[#0F172A] placeholder-slate-400 outline-none focus:border-[#2563EB] focus:bg-white transition disabled:opacity-50 disabled:cursor-not-allowed"
                        style="max-height: 100px; overflow-y: auto;"
                        oninput="autoResize(this)"
                        onkeydown="handleEnter(event)"
                    ></textarea>
                </div>

                <button onclick="sendMessage()"
                    {{ !$active ? 'disabled' : '' }}
                    class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-linear-to-br from-[#2563EB] to-[#1149C7] text-white shadow-lg shadow-blue-500/20 hover:opacity-90 active:scale-95 transition disabled:opacity-40 disabled:cursor-not-allowed">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M3.478 2.405a.75.75 0 00-.926.94l2.432 7.905H13.5a.75.75 0 010 1.5H4.984l-2.432 7.905a.75.75 0 00.926.94 60.519 60.519 0 0018.445-8.986.75.75 0 000-1.218A60.517 60.517 0 003.478 2.405z"/>
                    </svg>
                </button>

            </div>

            <div class="mt-2.5 flex items-center justify-center gap-2">
                <p class="text-[11px] text-slate-400">Lebih nyaman di luar platform?</p>
                <a href="https://wa.me/" target="_blank"
                   class="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-600 hover:bg-emerald-100 transition">
                    <svg class="h-3 w-3" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                    </svg>
                    Lanjut di WhatsApp
                </a>
            </div>
        </div>

    </div>

</div>

<script>
const CSRF   = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
const MY_ID  = {{ auth()->id() }};
let activeConvId  = {{ $active ? $active->id : 'null' }};
let lastMessageId = 0;
let pollTimer     = null;

// ── Render a single message bubble ──────────────────────────────────────
function renderMessage(msg) {
    const wrap = document.createElement('div');
    if (msg.is_mine) {
        wrap.className = 'flex items-end justify-end gap-2.5 msg-bubble';
        wrap.innerHTML = `
            <div class="max-w-[65%]">
                <div class="rounded-3xl rounded-br-lg bg-linear-to-br from-[#2563EB] to-[#1149C7] px-4 py-3 shadow-lg shadow-blue-500/20">
                    <p class="text-sm leading-6 text-white whitespace-pre-wrap">${escHtml(msg.body)}</p>
                </div>
                <p class="mt-1 mr-2 text-right text-[10px] text-[#94A3B8]">${msg.time}</p>
            </div>`;
    } else {
        wrap.className = 'flex items-end gap-2.5 msg-bubble';
        wrap.innerHTML = `
            <div class="h-8 w-8 rounded-full bg-[#EAF2FF] flex items-center justify-center text-[#1846A3] font-bold text-xs flex-shrink-0 mb-1">
                ${escHtml(msg.sender_name.substring(0,2).toUpperCase())}
            </div>
            <div class="max-w-[65%]">
                <div class="rounded-3xl rounded-bl-lg bg-white border border-[#DCE7FB] px-4 py-3 shadow-sm">
                    <p class="text-sm leading-6 text-[#0F172A] whitespace-pre-wrap">${escHtml(msg.body)}</p>
                </div>
                <p class="mt-1 ml-2 text-[10px] text-[#94A3B8]">${msg.time}</p>
            </div>`;
    }
    return wrap;
}

// ── Load all messages for a conversation ────────────────────────────────
async function loadMessages(convId) {
    const inner = document.getElementById('messages-inner');
    if (!inner) return;

    const res  = await fetch(`/chat/conversations/${convId}/messages`, {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    });
    const msgs = await res.json();

    inner.innerHTML = '';

    if (msgs.length === 0) {
        inner.innerHTML = `
            <div class="flex items-center gap-3">
                <div class="flex-1 h-px bg-[#E2E8F0]"></div>
                <span class="text-xs font-semibold text-[#94A3B8]">Mulai percakapan</span>
                <div class="flex-1 h-px bg-[#E2E8F0]"></div>
            </div>`;
    } else {
        // Date separator
        inner.innerHTML = `
            <div class="flex items-center gap-3">
                <div class="flex-1 h-px bg-[#E2E8F0]"></div>
                <span class="text-xs font-semibold text-[#94A3B8]">Pesan</span>
                <div class="flex-1 h-px bg-[#E2E8F0]"></div>
            </div>`;
        msgs.forEach(msg => {
            inner.appendChild(renderMessage(msg));
            lastMessageId = Math.max(lastMessageId, msg.id);
        });
    }

    scrollToBottom();
}

// ── Poll for new messages ────────────────────────────────────────────────
async function pollMessages() {
    if (!activeConvId) return;
    try {
        const res  = await fetch(`/chat/conversations/${activeConvId}/messages?after=${lastMessageId}`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
        const msgs = await res.json();
        if (msgs.length > 0) {
            const inner = document.getElementById('messages-inner');
            msgs.forEach(msg => {
                inner.appendChild(renderMessage(msg));
                lastMessageId = Math.max(lastMessageId, msg.id);
                // Update conversation list preview
                updateConvPreview(activeConvId, msg.body);
            });
            scrollToBottom();
        }
    } catch(e) { /* ignore network errors */ }
}

// ── Switch to a different conversation ──────────────────────────────────
function openConversation(convId) {
    // Highlight in contact list
    document.querySelectorAll('.contact-item').forEach(el => {
        el.classList.remove('bg-[#EAF2FF]', 'border-l-2', 'border-l-[#2563EB]');
    });
    const btn = document.getElementById('contact-' + convId);
    if (btn) {
        btn.classList.add('bg-[#EAF2FF]', 'border-l-2', 'border-l-[#2563EB]');
        // Remove unread badge
        const badge = document.getElementById('badge-' + convId);
        if (badge) badge.remove();
    }

    activeConvId  = convId;
    lastMessageId = 0;

    // Navigate to the conversation (reload with ?c=id)
    window.location.href = `/chat?c=${convId}`;
}

// ── Send a message ───────────────────────────────────────────────────────
async function sendMessage() {
    if (!activeConvId) return;
    const input = document.getElementById('message-input');
    const text  = input.value.trim();
    if (!text) return;

    input.value = '';
    input.style.height = 'auto';

    // Optimistic render
    const inner = document.getElementById('messages-inner');
    const now   = new Date();
    const time  = now.getHours().toString().padStart(2,'0') + ':' + now.getMinutes().toString().padStart(2,'0');
    const tempMsg = { body: text, is_mine: true, sender_name: '', time };
    inner.appendChild(renderMessage(tempMsg));
    scrollToBottom();

    try {
        const res = await fetch(`/chat/conversations/${activeConvId}/messages`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': CSRF,
                'X-Requested-With': 'XMLHttpRequest'
            },
            body: JSON.stringify({ body: text })
        });
        const msg = await res.json();
        lastMessageId = Math.max(lastMessageId, msg.id);
        updateConvPreview(activeConvId, text);
    } catch(e) { /* ignore */ }
}

// ── Update conversation list last message preview ────────────────────────
function updateConvPreview(convId, text) {
    const btn = document.getElementById('contact-' + convId);
    if (!btn) return;
    const preview = btn.querySelector('p.text-xs.text-slate-500');
    if (preview) preview.textContent = text.length > 40 ? text.slice(0,40) + '…' : text;
    const timeEl = btn.querySelector('span.text-\\[10px\\]');
    if (timeEl) { const now = new Date(); timeEl.textContent = now.getHours().toString().padStart(2,'0') + ':' + now.getMinutes().toString().padStart(2,'0'); }
}

// ── Filter conversation list ─────────────────────────────────────────────
function filterContacts(query) {
    const q = query.toLowerCase();
    document.querySelectorAll('.contact-item').forEach(el => {
        el.style.display = el.dataset.name.includes(q) ? '' : 'none';
    });
}

// ── Helpers ──────────────────────────────────────────────────────────────
function autoResize(el) {
    el.style.height = 'auto';
    el.style.height = Math.min(el.scrollHeight, 100) + 'px';
}

function handleEnter(e) {
    if (e.key === 'Enter' && !e.shiftKey) {
        e.preventDefault();
        sendMessage();
    }
}

function escHtml(text) {
    return String(text)
        .replace(/&/g,'&amp;').replace(/</g,'&lt;')
        .replace(/>/g,'&gt;').replace(/"/g,'&quot;');
}

function scrollToBottom() {
    const el = document.getElementById('chat-messages');
    if (el) el.scrollTop = el.scrollHeight;
}

// ── Bootstrap ────────────────────────────────────────────────────────────
if (activeConvId) {
    loadMessages(activeConvId);
    pollTimer = setInterval(pollMessages, 2500);
}
</script>

</body>
</html>
